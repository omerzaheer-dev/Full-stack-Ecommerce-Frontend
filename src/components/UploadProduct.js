...
const UploadProduct = ({onClose}) => {
  ...
  useEffect(()=>{
    if(data.productName === "" || data.price === "" || data.description === "" || data.brand === "" || data.category === ""){
      ...
      selectedImages.length === 0 ? (<p>
        ...
        <div className={`gap-4 grid grid-cols-4 ${selectedImages.length > 0 ? "p-2 border-slate-100 border-[1px]":"p-0"}`}>
          {selectedImages.map((image, index) => (
            <div className='w-[100px] h-[106px] relative' key={index}>
              <img src={image} alt={`Selected ${index}`} className='w-full rounded-sm h-full bg-cover' />
              <div className='absolute top-1 font-bold right-1 bg-black text-white rounded-full cursor-pointer p-[1px]' onClick={()=>disSelectImage(index)}>
                <IoMdClose />
              </div>
            </div>
          ))}
        </div>
        <button className='px-20 py-2 mt-1 bg-red-600 rounded-md text-white font-medium font-serif text-xl hover:scale-105 transition-all hover:bg-red-700 block mx-auto'>Upload Product</button>
      </form>
      </div>
    </div>
  )
